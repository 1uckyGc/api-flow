...
- **AI Provider（双协议并存）** — `services/ai_service.py::AIClient.generate()` 是**唯一入口**，按 `settings.AI_PROVIDER` 分发到下面两个分支之一。两条分支返回同一种 `GenerationResult`，worker 不感知协议差异。
  - **HOLO API**（推荐，`AI_PROVIDER=holo`）— `_generate_holo`：异步**提交→轮询→下载**三步走（`POST /v1/generate` 拿 `task_id`，每 `AI_POLL_INTERVAL`（默认 5s）`GET /v1/tasks/{id}` 直到 `completed`/`failed`/`cancelled`/超时，再 `GET /v1/tasks/{id}/file` 取二进制）。失败任务自动退款，状态翻 `failed` 时 `result._terminal=True`，让 `generate_with_retry` 直接早退。
  - **Flow2API**（旧自建，`AI_PROVIDER=flow2api`）— `_generate_flow2api`：OpenAI 兼容 SSE 流式 `POST /v1/chat/completions`，从 markdown 图像 base64 或 URL 抠出二进制。
- **`LEGACY_MODEL_ALIASES`** — `ai_service.py` 模块级常量，把 Flow2API 时代的旧模型名（`*_ultra` / `*_ultra_relaxed` / `*_ultra_fl`）翻译成 HOLO 实名。仅 `_generate_holo` 入口处调 `normalize_holo_model(model)`。映射规则：`_ultra_relaxed` → `lite_*`，`_ultra` / `_ultra_fl` → `fast_*`。前端 `useProvider()` + `frontend/src/constants/models.js` 已能按 provider 直接发实名，alias 表是**兜底防御**，确保旧的 schema default、保存过的 task config_json、误传值都能跑通。
- **DeepSeek** — 提示词扩写（裂变）和剧本拆分（导演）用的 LLM。系统级配置：`DEEPSEEK_API_KEY` / `DEEPSEEK_API_URL` / `DEEPSEEK_MODEL`。请求带 `response_format: json_object`。**与 provider 切换无关**，永远直连 `api.deepseek.com`。
...
- **生成结果** — `GenerationResult` 是内存结构：`{success, media_type, data: bytes, mime_type, error, file_ext, _terminal?}`。HOLO 分支按 `result.file_ext` 字段直接确定后缀；Flow2API 分支从 SSE 响应里解（markdown 图像语法里的 base64 data-URL、文本中的可下载 URL、原始流内容）。两边都靠 magic bytes 兜底，**不信 Content-Type**。
...
- **重试分两层** — `generate_with_retry` 做**生成层**重试，指数退避（`2^attempt`）。`generate` 内部的**下载阶段**有自己独立的 3 次重试（针对 `RemoteProtocolError / ReadError / ConnectError`）。**别混为一谈**。HOLO 分支 polled 失败 / 取消时挂 `result._terminal=True`，`generate_with_retry` 看到立即返回，不进重试循环（HOLO 已自动退款，重试只会重复烧配额）。
...
- **`schemas/task.py::DirectorCreateRequest.video_model` 默认还是 Flow2API 时代的 `veo_3_1_i2v_s_fast_portrait_ultra_relaxed`。** 不是 bug，HOLO 模式下走 `LEGACY_MODEL_ALIASES` 自动翻译为 `veo_3_1_i2v_lite_portrait`；Flow2API 模式直接吃。前端总会传具体值过来覆盖，default 仅在请求体省略时兜底。
- **图片上传校验改成「按 magic 推真实后缀」，不再拒绝伪造文件。** `routers/upload.py::_detect_real_ext` 看头几个字节决定保存名（e.g. JPEG 内容的 `001.png` 入库存为 `<uuid>.jpg`）。这是为了容忍用户那些后缀错乱的素材，不是安全削弱 —— 仅当完全识别不出图片格式（不是 JPG/PNG/WebP/GIF）才 400 拒。
