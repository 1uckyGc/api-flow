...
- **dreamina success 后直链下载（绕开 CLI 慢路径，2026-05-12）**。dreamina CLI 自带的 `query_result --download_dir=` 二次 subprocess 下载 11.5MB 实测 **25.7s**（~460KB/s），同一 CDN URL 用 `curl` 只要 **2s**（~5.7MB/s）—— CLI 内部下载实现 12x slow。修：
  - 新 helper `_parse_query_result_json(text)`：dreamina CLI stdout 偶尔混 SQL 慢查询 ANSI 日志在 JSON 前面（多个 worker thread 并发跑 query_result 时会触发），用 marker `{\n  "submit_id"` + 括号配平定位真正的 JSON 块；直接 `json.loads(text)` 会因前缀噪声炸
  - 新 helper `_extract_video_url_from_query(text)`：从解析后的 JSON 抽 `result_json.videos[0].video_url`（CDN 直链含签名 token）
  - 新 helper `_direct_download_video(url, ddir, sid)`：`httpx.Client(...).stream("GET", url)` 64KB chunk 流式落盘到 `ddir/<sid>_video_1.mp4`
  - 3 处 `gs == "success"` 路径（`image2video` / `multimodal2video` / `_submit_poll_download`）全部改为**先尝试直链下载，失败 fallback** 老的 `query_result --download_dir=` CLI 路径
  - **状态变化日志补全**：以前 `if last_queue_idx != qi: logger.info(...)` 只在 queue_idx 变化才打 log，导致 `queue=0 → success` 转换阶段（通常 1-4min 渲染期）完全沉默。改成 `if gs != last_gs or queue_idx changed` 任一变都 log，能看到 success transition 时刻
...
- **cc123 仅 text-to-video，OpenAI Sora-compat 形态**（2026-05-12 探测确认）。cc123 model list 只暴露 `sd-2` (0.57 USD/call) 和 `sd-2-vip` (0.65 USD/call)，**没有 sd-2-omniref / sd-2-i2v / 首尾帧变体**。我们的客户端 payload 只发 `{model, prompt, duration, orientation, size, watermark}` —— 即使传 `image / first_frame / reference_image` 也会被 NewAPI relay silently 吞掉（不识别字段不报错），上游 Sora-compat 协议本身 schema 不含 image 参数。**全能参考唯一路径**是 `dreamina_client.multimodal2video()`（前端模型 ID `dreamina/seedance2.0fast-omniref` 等）。
- **cc123 `completed but content download failed`**（2026-05-12 已诊断，未修）。源：`cc123_video_client.py:224`。触发条件：cc123 poll 说 `status=completed`，但 `GET /v1/videos/{id}/content` 返 **502 `Upstream service returned status 500`** —— cc123 → Volcengine 后端 state inconsistency（cc123 标完成但 Volcengine 这边视频文件没真存住或被 purge）。实测同一 task_id 几小时后仍 502，文件永久丢失，扣费已成定局。`metadata.url` 字段 = `https://cc123.ai/v1/videos/{id}/content` 同一 endpoint，没"直链 fallback"路径。**当前没修**；未来可加 download retry（3-5 次 exp backoff）+ task.config_json 存 cc123_task_id 便于 ops 复盘。
- **僵尸任务每小时自动清 + 按钮也清**。worker 崩溃/重启/网络断会留下 `status=running` 或 `queued` 永远不更新的"卡住"任务（UI 一直显示"生成中..."）。两层兜底：
