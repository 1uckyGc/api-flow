...
| **AI 网关** | Flow2API（OpenAI 兼容协议）调 Gemini（图）/ Veo（视频）；DeepSeek 做提示词扩写 |
...
- **Key 分两级**：Gemini / Veo key 是按用户存在 `system_settings` 里的；DeepSeek key 是系统级的（`.env`）。`_get_user_api_key` 按模型名关键字选用户那把。
